...
### 🚀 테스트 리포트 주소: https://albamon.wooyoung.site (AI Challenge 종료로 인해 배포 중단)
...
### ✅ 결과물 검토 및 보완 설명
1. 비정상적인 ID 와 PW 를 입력후 로그인 버튼을 클릭하면 /member/login 으로 요청후 응답값의 message로 alert를 표시해주었는데 AI의 경우 해당 로직을 작성해주지 않았습니다.
2. alert의 문자를 비교할때 contains를 사용해서 해당 문자열이 정확하게 원하는 값인지를 체크하지 않고 포함되어 있는지로 체크하고 있엇습니다.
3. alert에 대해서 리스너를 등록해주지 않았습니다.
4. 로그인 시 마이페이지를 통해 로그인을 해야 정상 로그인 후 마이페이로 이동하는데 만약 마이페이지가 아닌 다른경로를 통해서 (미 로그인 상태에서 알바 지원) 로그인을 해본다든가 이런 엣지케이스에 대해서는 검증하지 않았습니다.
